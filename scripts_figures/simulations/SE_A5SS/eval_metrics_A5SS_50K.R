...
dxr1_exon17 <- dxr1 %>% 
  filter(featureID=="E017")

dxr1_exon17 <- dxr1_exon17 %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
dxr1_exon17$ReadDepth <- as.numeric(gsub("RD","", dxr1_exon17$ReadDepth))
dxr1_exon17$FoldChange <- gsub("FC","", dxr1_exon17$FoldChange)
dxr1_exon17$FoldChange <- gsub("minus","-", dxr1_exon17$FoldChange)
dxr1_exon17$FoldChange <- gsub("plus","", dxr1_exon17$FoldChange)
dxr1_exon17$FoldChange <- as.numeric(dxr1_exon17$FoldChange)

dexseq_FC_RD_freq <- data.frame(table(dxr1_exon17$FoldChange, dxr1_exon17$ReadDepth))
dexseq_FC_RD_freq$ID <- "DEXSeq"


jcec <- read.table("/mnt/storage/jaquino/polyester_sim_2/SE_A5SS/A5SS_disp3_rmats/A5SS.MATS.JCEC.txt", header=TRUE, sep="\t")
jcec <- jcec %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
...
jc <- read.table("/mnt/storage/jaquino/polyester_sim_2/SE_A5SS/A5SS_disp3_rmats/A5SS.MATS.JC.txt", header=TRUE, sep="\t")
jc <- jc %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
...
dxr1_exon17$sig.pvalue <- NA
for(i in 1:nrow(dxr1_exon17)){
  if(is.na(dxr1_exon17$pvalue[i])){
    next
  }
  if(dxr1_exon17$pvalue[i] <= 0.05){
    dxr1_exon17$sig.pvalue[i] <- "+"
  }
  if(dxr1_exon17$pvalue[i] > 0.05){
    dxr1_exon17$sig.pvalue[i] <- "-"
  }
...
dxr1_exon17$sig.adjpvalue <- NA
for(i in 1:nrow(dxr1_exon17)){
  if(is.na(dxr1_exon17$corrected.padj[i])){
    next
  }
  if(dxr1_exon17$corrected.padj[i] <= 0.05){
    dxr1_exon17$sig.adjpvalue[i] <- "+"
  }
  if(dxr1_exon17$corrected.padj[i] > 0.05){
    dxr1_exon17$sig.adjpvalue[i] <- "-"
  }
...
dexseq_jcec_merge <- merge(dxr1_exon17, jcec, by=c("FoldChange", "ReadDepth", "SimRun"))
dexseq_jc_merge <- merge(dxr1_exon17, jc, by=c("FoldChange", "ReadDepth", "SimRun"))
...
ggplot(specificity_tbl, aes(group=var, y=val, x=ReadDepth)) +
  ...
  labs(title="A5SS Specificity raw pvalue", x="Read Depth", y="Specificity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw()
ggsave("Spec_pvalue_lineplot_A5SS.pdf", width = 6, height = 5, dpi = 100, units = "in", device='pdf')
...
ggplot(sens_tbl, aes(group=var, y=val, x=ReadDepth)) +
  ...
  labs(title="A5SS Sensitivity raw pvalue", x="Read Depth", y="Sensitivity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw() +
  facet_wrap(~FoldChange, nrow=2)
ggsave("Sens_lineplot_A5SS.pdf", width = 18, height = 7, dpi = 100, units = "in", device='pdf')
...
ggplot(sens_tbl, aes(group=var, y=val, x=ReadDepth)) +
  geom_line(aes(color=var), position = position_dodge(width = 0.2)) +
  labs(title="A5SS Sensitivity adjusted pvalue", x="Read Depth", y="Sensitivity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw() +
  facet_wrap(~FoldChange, nrow=2)
ggsave("Sens_adjpvalue_lineplot_A5SS.pdf", width = 18, height = 7, dpi = 100, units = "in", device='pdf')
...
ggplot(specificity_tbl, aes(group=var, y=val, x=ReadDepth)) +
  geom_line(aes(color=var), position = position_dodge(width = 0.1)) +
  labs(title="A5SS Specificity adjusted pvalue", x="Read Depth", y="Specificity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw()
ggsave("Spec_adjpvalue_lineplot_A5SS.pdf", width = 6, height = 5, dpi = 100, units = "in", device='pdf')
