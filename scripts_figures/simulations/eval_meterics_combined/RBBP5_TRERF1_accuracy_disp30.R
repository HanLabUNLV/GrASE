#raw pvalue
sens_raw <- read.table("/mnt/storage/jaquino/polyester_sim_2/A5SS_RBBP5_TRERF1_res/Sens_pvalue_A5SS_RBBP5_TRERF1_disp30.txt", header=TRUE, sep="\t")
spec_raw <- read.table("/mnt/storage/jaquino/polyester_sim_2/A5SS_RBBP5_TRERF1_res/Spec_pvalue_A5SS_RBBP5_TRERF1_disp30.txt", header=TRUE, sep="\t")
...
write.table(accuracy_combine, "/mnt/storage/jaquino/polyester_sim_2/A5SS_RBBP5_TRERF1_res/Acc_pvalue_A5SS_RBBP5_TRERF1_disp30.txt", 
...
ggplot(accuracy_combine, aes(group=var2, y=val, x=factor(ReadDepth))) +
  ...
  labs(title="A5SS Accuracy raw p-value", x="Read Depth", y="Accuracy") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1) +
  theme_bw(base_size = 20) +
  facet_wrap(~FoldChange, nrow=2)
ggsave("/mnt/storage/jaquino/polyester_sim_2/A5SS_RBBP5_TRERF1_res/Acc_pvalue_lineplot_A5SS_RBBP5_TRERF1_disp30.pdf", width = 28, height = 12, dpi = 100, units = "in", device='pdf')

#adjusted p-value
sens_adj <- read.table("/mnt/storage/jaquino/polyester_sim_2/A5SS_RBBP5_TRERF1_res/Sens_pvalue_A5SS_RBBP5_TRERF1_disp30_adj.txt", header=TRUE, sep="\t")
spec_adj <- read.table("/mnt/storage/jaquino/polyester_sim_2/A5SS_RBBP5_TRERF1_res/Spec_pvalue_A5SS_RBBP5_TRERF1_disp30_adj.txt", header=TRUE, sep="\t")
...
write.table(accuracy_combine, "/mnt/storage/jaquino/polyester_sim_2/A5SS_RBBP5_TRERF1_res/Acc_pvalue_A5SS_RBBP5_TRERF1_disp30_adj.txt", 
            sep="\t", quote=FALSE, row.names = FALSE)

accuracy_combine$FoldChange <- factor(accuracy_combine$FoldChange, levels=c(-8, -4, -2, -1, 8, 4, 2, 1))
ggplot(accuracy_combine, aes(group=var2, y=val, x=factor(ReadDepth))) +
  geom_line(aes(linetype=gene, color=var), alpha = 0.55, size =1, position = position_dodge(width = 0.2)) +
  labs(title="A5SS Accuracy adjusted p-value", x="Read Depth", y="Accuracy") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1) +
  theme_bw(base_size = 20) +
  facet_wrap(~FoldChange, nrow=2)
ggsave("/mnt/storage/jaquino/polyester_sim_2/A5SS_RBBP5_TRERF1_res/Acc_pvalue_lineplot_A5SS_RBBP5_TRERF1_adj_disp30.pdf", width = 28, height = 12, dpi = 100, units = "in", device='pdf')
