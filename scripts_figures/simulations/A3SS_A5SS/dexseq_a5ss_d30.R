...
flattenedFile = list.files("/scratch/han_lab/dwito/A3SS_A5SS_D30/dexseq/A5SS", pattern="combined_ENSG00000124496.12.gff$", full.names=TRUE) #read in dexseq annotation file
sampleTable = data.frame(
  ...
  condition = c( "control", "control", "control", "control", "control", 
                 "case_a5", "case_a5", "case_a5", "case_a5", "case_a5"))
countFiles = list.files("/scratch/han_lab/dwito/A3SS_A5SS_D30/dexseq/A5SS/samples", pattern="dexseq.cnts.txt$", full.names=TRUE)
...
write.table(dxr1,"A5SS_dexseq_results_50K_sim.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
...
write.table(dexseq_successful,"A5SS_dexseq_corrected_padj.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
