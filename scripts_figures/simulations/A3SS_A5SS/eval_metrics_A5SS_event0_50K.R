...
dxr1_exon15 <- dxr1_exon15 %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
dxr1_exon15$ReadDepth <- as.numeric(gsub("RD","", dxr1_exon15$ReadDepth))
dxr1_exon15$FoldChange <- gsub("FC","", dxr1_exon15$FoldChange)
dxr1_exon15$FoldChange <- gsub("minus","-", dxr1_exon15$FoldChange)
dxr1_exon15$FoldChange <- gsub("plus","", dxr1_exon15$FoldChange)
dxr1_exon15$FoldChange <- as.numeric(dxr1_exon15$FoldChange)

dexseq_FC_RD_freq <- data.frame(table(dxr1_exon15$FoldChange, dxr1_exon15$ReadDepth))
dexseq_FC_RD_freq$ID <- "DEXSeq"

#rMATS simulation runs - A5SS event 0 JCEC & JC
jcec <- read.table("/scratch/han_lab/dwito/A3SS_A5SS_D30/rmats/A5SS/event_0/A5SS.MATS.JCEC.txt", header=TRUE, sep="\t")
jcec <- jcec %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
...
jc <- read.table("/scratch/han_lab/dwito/A3SS_A5SS_D30/rmats/A5SS/event_0/A5SS.MATS.JC.txt", header=TRUE, sep="\t")
jc <- jc %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
...
########################################################################################################################
# dexseq exon 16 (spliced exon) table
dxr1_exon15$sig.pvalue <- NA
for(i in 1:nrow(dxr1_exon15)){
  if(is.na(dxr1_exon15$pvalue[i])){
    next
  }
  if(dxr1_exon15$pvalue[i] <= 0.05){
    dxr1_exon15$sig.pvalue[i] <- "+"
  }
  if(dxr1_exon15$pvalue[i] > 0.05){
    dxr1_exon15$sig.pvalue[i] <- "-"
  }
...
dxr1_exon15$sig.adjpvalue <- NA
for(i in 1:nrow(dxr1_exon15)){
  if(is.na(dxr1_exon15$corrected.padj[i])){
    next
  }
  if(dxr1_exon15$corrected.padj[i] <= 0.05){
    dxr1_exon15$sig.adjpvalue[i] <- "+"
  }
  if(dxr1_exon15$corrected.padj[i] > 0.05){
    dxr1_exon15$sig.adjpvalue[i] <- "-"
  }
...
dexseq_jcec_merge <- merge(dxr1_exon15, jcec, by=c("FoldChange", "ReadDepth", "SimRun"))
dexseq_jc_merge <- merge(dxr1_exon15, jc, by=c("FoldChange", "ReadDepth", "SimRun"))
