...
#DEXSeq simulation runs - combined A5SS
dxr1 <- read.table("/scratch/han_lab/dwito/A3SS_A5SS_D30/dexseq/A5SS/A5SS_dexseq_corrected_padj.txt", header=TRUE, sep="\t")
...
dxr1_exon23 <- dxr1 %>% 
  filter(featureID=="E023")

dxr1_exon23 <- dxr1_exon23 %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
dxr1_exon23$ReadDepth <- as.numeric(gsub("RD","", dxr1_exon23$ReadDepth))
dxr1_exon23$FoldChange <- gsub("FC","", dxr1_exon23$FoldChange)
dxr1_exon23$FoldChange <- gsub("minus","-", dxr1_exon23$FoldChange)
dxr1_exon23$FoldChange <- gsub("plus","", dxr1_exon23$FoldChange)
dxr1_exon23$FoldChange <- as.numeric(dxr1_exon23$FoldChange)

dexseq_FC_RD_freq <- data.frame(table(dxr1_exon23$FoldChange, dxr1_exon23$ReadDepth))
dexseq_FC_RD_freq$ID <- "DEXSeq"

#rMATS simulation runs - A5SS event 1 JCEC & JC
jcec <- read.table("/scratch/han_lab/dwito/A3SS_A5SS_D30/rmats/A5SS/event_1/A5SS.MATS.JCEC.txt", header=TRUE, sep="\t")
jcec <- jcec %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
...
jc <- read.table("/scratch/han_lab/dwito/A3SS_A5SS_D30/rmats/A5SS/event_1/A5SS.MATS.JC.txt", header=TRUE, sep="\t")
jc <- jc %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
...
#total simulations
folders_50K_A5SS <- read.table("/scratch/han_lab/dwito/A3SS_A5SS_D30/folders_50K_A5SS.txt")
total_sim <- folders_50K_A5SS %>% 
...
########################################################################################################################
# dexseq exon 24 (spliced exon) table
dxr1_exon23$sig.pvalue <- NA
for(i in 1:nrow(dxr1_exon23)){
  if(is.na(dxr1_exon23$pvalue[i])){
    next
  }
  if(dxr1_exon23$pvalue[i] <= 0.05){
    dxr1_exon23$sig.pvalue[i] <- "+"
  }
  if(dxr1_exon23$pvalue[i] > 0.05){
    dxr1_exon23$sig.pvalue[i] <- "-"
  }
...
dxr1_exon23$sig.adjpvalue <- NA
for(i in 1:nrow(dxr1_exon23)){
  if(is.na(dxr1_exon23$corrected.padj[i])){
    next
  }
  if(dxr1_exon23$corrected.padj[i] <= 0.05){
    dxr1_exon23$sig.adjpvalue[i] <- "+"
  }
  if(dxr1_exon23$corrected.padj[i] > 0.05){
    dxr1_exon23$sig.adjpvalue[i] <- "-"
  }
...
dexseq_jcec_merge <- merge(dxr1_exon23, jcec, by=c("FoldChange", "ReadDepth", "SimRun"))
dexseq_jc_merge <- merge(dxr1_exon23, jc, by=c("FoldChange", "ReadDepth", "SimRun"))
...
#A5SS false positives and True Negatives
#specificity
DEXSeqTN=c()
DEXSeqFP=c()
rMATS_jcecTN=c()
...
#A5SS false negatives and true positives
