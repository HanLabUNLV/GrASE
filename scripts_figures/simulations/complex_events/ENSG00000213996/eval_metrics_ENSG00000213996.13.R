...
dxr1 <- read.table("/home/dwito/GrASE_package/SplicingGraphs/candidate_genes/_simulations/dexseq/ENSG00000213996.13/ENSG00000213996.13_dexseq_corrected_padj.txt", header=TRUE, sep="\t")
...
jcec <- read.table("/home/dwito/GrASE_package/SplicingGraphs/candidate_genes/_simulations/rmats/ENSG00000213996.13/A3SS.MATS.JCEC.txt", header=TRUE, sep="\t")
jcec <- jcec %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
...
jc <- read.table("/home/dwito/GrASE_package/SplicingGraphs/candidate_genes/_simulations/rmats/ENSG00000213996.13/A3SS.MATS.JC.txt", header=TRUE, sep="\t")
jc <- jc %>% 
  separate_wider_delim(ID, "_", names = c("FoldChange", "ReadDepth", "SimRun"))
...
#total simulations
folders_50K_A3SS <- read.table("/home/dwito/GrASE_package/SplicingGraphs/candidate_genes/_simulations/ENSG00000213996.13/folders_50K.txt")
...
for(i in unique(mixedsort(dexseq_jc_merge$ReadDepth))){
  ...
  if(length(dexseq_2way_tbl)==2){
    DEXSeqFP <- c(DEXSeqFP, dexseq_2way_tbl[2])
  ...
  if(length(rmatsjc_2way_tbl)==2){
    rMATS_jcFP <- c(rMATS_jcFP, rmatsjc_2way_tbl[2])
  ...
  if(length(rmatsjcec_2way_tbl)==2){
    rMATS_jcecFP <- c(rMATS_jcecFP, rmatsjcec_2way_tbl[2])
...
ggplot(specificity_tbl, aes(group=var, y=val, x=ReadDepth)) +
  ...
  labs(title="ENSG00000213996.13 A3SS Specificity raw pvalue", x="Read Depth", y="Specificity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw() 
ggsave("Spec_pvalue_lineplot.pdf", width = 10, height = 5, dpi = 100, units = "in", device='pdf')
...
for(FC in Foldchanges){
  ...
  for(i in unique(mixedsort(dexseq_jc_merge$ReadDepth))){
    ...
    dexseq_2way_tbl <- table(merged_filt$sig.pvalue.x)
    if(length(dexseq_2way_tbl)==2){
      DEXSeqTP <- c(DEXSeqTP, dexseq_2way_tbl[2])
    ...
    rmatsjc_2way_tbl <- table(merged_filt$sig.pvalue.y)
    if(length(rmatsjc_2way_tbl)==2){
      rMATS_jcTP <- c(rMATS_jcTP, rmatsjc_2way_tbl[2])
    ...
    rmatsjcec_2way_tbl <- table(merged_filt$sig.pvalue.y)
    if(length(rmatsjcec_2way_tbl)==2){
      rMATS_jcecTP <- c(rMATS_jcecTP, rmatsjcec_2way_tbl[2])
...
ggplot(sens_tbl, aes(group=var, y=val, x=ReadDepth)) +
  ...
  labs(title="ENSG00000213996.13 A3SS Sensitivity raw pvalue", x="Read Depth", y="Sensitivity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw() +
  facet_wrap(~FoldChange, nrow=2)
ggsave("Sens_lineplot.pdf", width = 25, height = 7, dpi = 100, units = "in", device='pdf')
...
ggplot(sens_tbl, aes(group=var, y=val, x=ReadDepth)) +
  geom_line(aes(color=var), position = position_dodge(width = 0.2)) +
  labs(title="ENSG00000213996.13 A3SS Sensitivity adjusted pvalue", x="Read Depth", y="Sensitivity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw() +
  facet_wrap(~FoldChange, nrow=2)
ggsave("Sens_adjpvalue_lineplot.pdf", width = 25, height = 7, dpi = 100, units = "in", device='pdf')
...
ggplot(specificity_tbl, aes(group=var, y=val, x=ReadDepth)) +
  geom_line(aes(color=var), position = position_dodge(width = 0.1)) +
  labs(title="ENSG00000213996.13 A3SS Specificity adjusted pvalue", x="Read Depth", y="Specificity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw()
ggsave("Spec_adjpvalue_lineplot.pdf", width = 10, height = 5, dpi = 100, units = "in", device='pdf')
