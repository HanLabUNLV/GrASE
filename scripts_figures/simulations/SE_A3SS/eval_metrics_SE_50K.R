...
library(tidyverse)
setwd("/mnt/storage/jaquino/polyester_sim_2/SE_A3SS/dexseq_disp3")

folders_50K <- read.table("/mnt/storage/jaquino/polyester_sim_2/SE_A3SS/folders_ID_50K.txt", header=FALSE, sep="\t")
...
ggplot(combined_sim , aes(x=Var2, y=Freq, fill=ID)) + 
  ...
  labs(title="Total Simulation Runs vs Successful DEXSeq and rMATS Runs", x="Read Depth") +
  theme_bw() +
  scale_fill_brewer(palette = "Dark2", name="Simulations") 
ggsave("simulation_dist.pdf", width = 14, height = 8, dpi = 100, units = "in", device='pdf') 

########################################################################################################################
# dexseq exon 2 (skipped exon) table
...
ggplot(specificity_tbl, aes(group=var, y=val, x=ReadDepth)) +
  ...
  labs(title="SE Specificity raw pvalue", x="Read Depth", y="Specificity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw()
ggsave("Spec_pvalue_lineplot_SE.pdf", width = 6, height = 5, dpi = 100, units = "in", device='pdf')
...
ggplot(sens_tbl, aes(group=var, y=val, x=ReadDepth)) +
  ...
  labs(title="SE Sensitivity raw pvalue", x="Read Depth", y="Sensitivity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw() +
  facet_wrap(~FoldChange, nrow=2)
ggsave("Sens_lineplot_SE.pdf", width = 18, height = 7, dpi = 100, units = "in", device='pdf')
...
ggplot(sens_tbl, aes(group=var, y=val, x=ReadDepth)) +
  geom_line(aes(color=var), position = position_dodge(width = 0.2)) +
  labs(title="SE Sensitivity adjusted pvalue", x="Read Depth", y="Sensitivity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw() +
  facet_wrap(~FoldChange, nrow=2)
ggsave("Sens_adjpvalue_lineplot_SE.pdf", width = 18, height = 7, dpi = 100, units = "in", device='pdf')
...
ggplot(specificity_tbl, aes(group=var, y=val, x=ReadDepth)) +
  geom_line(aes(color=var), position = position_dodge(width = 0.1)) +
  labs(title="SE Specificity adjusted pvalue", x="Read Depth", y="Specificity") +
  scale_color_discrete(name = "Software", labels=c("DEXSeq", "rMATS JC", "rMATS JCEC")) +
  ylim(0,1)+
  theme_bw()
ggsave("Spec_adjpvalue_lineplot_SE.pdf", width = 6, height = 5, dpi = 100, units = "in", device='pdf')
